...
with open(filename,'rb') as f:						# Open the file with stored data
	u,rs,mu,kopt,Topt = pickle.load(f)				# Load the data
	

fig, (ax1,ax2) = plt.subplots(2,1)					# Open a figure

for i in range(int(rs.size)):						# One plot for every value in u
	ax1.plot(mu,Topt[i],Label = 'r times s = ' + str(rs[i]))
	ax2.plot(mu,kopt[i],Label = 'r times s = ' + str(rs[i]))
